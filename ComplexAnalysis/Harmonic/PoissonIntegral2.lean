...
lemma tendsto_integral_prod_of_continuousOn
  ...
  have hrn (n : ℕ) (t : ℝ) : (r n) * exp (t * I) ∈ closedBall 0 1  := by
    exact ball_subset_closedBall (exp_ofReal_mul_I_in_unitDisc (hr n) t)
...
-- #count_heartbeats in -- 2200 hb
/-- The sequence `r_n = 1 - 1 / (n + 2)` tends to `1` as `n → ∞`. -/
...
satisfies `u(z) = (1/2π) ∫_0^{2π} ((e^{it} + z) / (e^{it} - z)).re u(e^{it}) dt`
for `z` in the unit disc. -/
